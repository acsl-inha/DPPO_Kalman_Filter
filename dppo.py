...
class ActorCritic(nn.Module):
    ...
    def act(self, state, memory):
        ...
        action_probs = self.action_layer(state).flatten()
    ...
    def evaluate(self, state, action):
        action_probs = self.action_layer(state.reshape(-1, 100))
        dist = Categorical(action_probs)

        action_logprobs = dist.log_prob(action)
        dist_entropy = dist.entropy()           # entropy for SAC

        state_value = self.value_layer(state.reshape(-1, 100))
...
class DPPO:
    ...
    def update(self, memory):
        ...
        for _ in range(self.K_epochs):
            ...
            logprobs, state_values, _ = self.policy.evaluate(
...
def train_agent(env, dppo, test_model, mean, std, memory, max_episodes, max_timesteps, value_tune_epi,
                update_timestep, log_interval, solved_reward, experiment_version, plot_option):
    # logging variables
    running_reward = 0
        ...
        for t in range(max_timesteps):
            ...
            with torch.no_grad():
                if not test_done:
                    if t == 0:
                        test_state = state
                    test_action = test_model(torch.from_numpy(test_state).view(1, -1).float().to(device)).flatten().max(0)[1].item()
            ...
            if i_episode < value_tune_epi:
                action = dppo.policy_old.act(state.reshape(1, -1), memory)
            else:
                if i_episode == value_tune_epi:
