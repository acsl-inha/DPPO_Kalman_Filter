...
class AircraftEnv(gym.Env):
    ...
    def __init__(self, succeed_coef=8000, collide_coef=-2000, change_cmd_penalty=-200, cmd_penalty=-1,
                 start_cond_coef=90, cmd_suit_coef=0.01, reset=False):
        ...
        self.state = deque(maxlen=20)  # initialize states
        self.h_cmd_suit = 0
        self.t_step = 0
        ...
        self.high = np.array(
            [2 * np.pi, 2 * np.pi, 2 * np.pi, 5000, 400] * 20, dtype=np.float32)
        self.action_space = spaces.Discrete(3)
        self.observation_space = spaces.Box(
            low=-self.high, high=self.high, dtype=np.float32)  # r, vc, los, daz, dlos

        # set initial states for 20 steps
        for _ in range(20):
            self.dynamics()
    ...
    def dynamics(self):
        ...
        # real rate of LOS
        self.dlos = (self.Pr_NED[2] * self.Vr_NED[0] - self.Pr_NED[0] * self.Vr_NED[2]) / \
                    (self.Pr_NED[0] ** 2 + self.Pr_NED[2] ** 2 + self.Pr_NED[1] ** 2)
        # real rate of azim
        self.daz = (self.Pr_NED[1] * self.Vr_NED[0] - self.Pr_NED[0] * self.Vr_NED[1]) / \
                   (self.Pr_NED[0] ** 2 + self.Pr_NED[1] ** 2 + self.Pr_NED[2] ** 2)

        self.los_p = self.los
        self.dlos_p = self.dlos
        self.azim_p = self.azim
        self.daz_p = self.daz
        self.state.append([self.theta + self.np_random.randn() * self.Deg2Rad,
                           self.elev + self.np_random.randn() * self.Deg2Rad,
                           self.azim + self.np_random.randn() * self.Deg2Rad,
                           self.r + 0.01 * self.r * self.np_random.randn(), self.vc + 0.001 * self.r * self.np_random.randn()])
    ...
    def step(self, action):
        ...
        elif self.t_step > 3 and self.r > self.dist_sep and abs(self.elev) > 40 * self.Deg2Rad and abs(
                self.azim) > 40 * self.Deg2Rad:
            reward = self.succeed_coef
            done = True
        ...
        return np.array(self.state), reward, done, {
            "info": [self.hdot_cmd, self.r, self.elev, self.azim, self.Pm_NED, self.Pt_NED, self.h, self.height_diff]}

    def reset(self):
        self.__init__(self.succeed_coef, self.collide_coef, self.change_cmd_penalty, self.cmd_penalty,
                      self.start_cond_coef, self.cmd_suit_coef, True)
